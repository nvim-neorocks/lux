    ...
    let non_entrypoints = packages
        ...
        .filter_map(|pkg_id| {
            if lockfile.is_entrypoint(pkg_id) {
                ...
                Some(unsafe { lockfile.get_known(pkg_id) }.name().to_string())
            }
        })
        .collect_vec();
...
",
    ...
    if !dependencies.is_empty() {
        let package_names = dependencies
            .iter()
            .map(|pkg_id| unsafe { lockfile.get_known(pkg_id) }.name().to_string())
            .collect_vec();
...
Reinstall?
            ...
            .expect("Error prompting for reinstall")
        {
            let reinstall_specs = dependencies
                .iter()
                .map(|pkg_id| {
                    let package = unsafe { lockfile.get_known(pkg_id) };
