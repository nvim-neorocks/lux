...
impl Manifest {
    ...
    pub async fn from_config(
        ...
        let content =
            crate::manifest::manifest_from_cache_or_server(&server_url, &manifest_version, config, progress).await?;
        match ManifestMetadata::new(&content) {
            Ok(metadata) => Ok(Self::new(server_url, metadata)),
            Err(_) => {
                let manifest =
                    crate::manifest::manifest_from_server_only(&server_url, &manifest_version, config, progress)
                        .await?;
