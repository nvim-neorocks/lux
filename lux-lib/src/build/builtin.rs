...
impl Build for BuiltinBuildSpec {
    ...
    async fn run(
        ...
        for target in autodetect_bin_scripts(build_dir) {
            std::fs::create_dir_all(target.parent().unwrap())?;
            let target = target.to_string_lossy().to_string();
            let source = build_dir.join("src").join("bin").join(&target);
            // Let's not pass in the rockspec's deploy field only for auto-detected bin scripts
            ...
            binaries.push(installed_bin_script);
        }
